...
export class BaMenuService {
  ...
  constructor(private _router:Router, private httpService: Http) { }
  ...
  public selectMenuItem(menuItems:any[], inputItem):any[] {
    let items = [];
    menuItems.forEach((item) => {
      this._selectItem(item);

      if (item.selected) {
        let id: string = item.route.id;
      ...
      if (inputItem && item.route.path == inputItem.route.path) {
        inputItem.selected = true;
        this._currentMenuItem = inputItem;
      }

      if (item.children && item.children.length > 0) {
        item.children = this.selectMenuItem(item.children, null);
      }
      items.push(item);
    });
    return items;
