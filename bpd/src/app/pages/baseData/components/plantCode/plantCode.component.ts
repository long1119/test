...
import 'style-loader!./plantCode.scss';
import {
  Message,
  SelectItem,
  TreeNode,
...
export class PlantCode {
  ...
  constructor(private service: plantCodeService, private msgService: MessageService, private dataManageService: DataManageService, private deleteConfirmService: DeleteComfirmService) {
    this.capitalExpense = [];
  ...
  ngOnInit() {
    ...
    this.localStorageAuthority = this.dataManageService.buttonAuthority("Maintain Plant Code");
  ...
  deleteClick(idx, data) {
    ...
    this.deleteConfirmService.confirm(() => {
